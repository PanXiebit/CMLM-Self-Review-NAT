...
class TransformerHparams(object):
    ...
    def get_encoder_flops(self, batch_size, train_steps):
        """Get the FLOPs for pre-training the transformer encoder."""
...
def get_cmlm_train_flops(
        h_d, l_d, h_g, l_g, batch_size, train_steps, tied_embeddings,
        e=None, s=512, v=34984, output_frac=0.5):
    if e is None:
        e = h_d
    enc = TransformerHparams(
        h=h_d, l=l_d, s=s, v=v, e=e, i=None, heads=None,
        head_size=None, output_frac=output_frac, sparse_embed_lookup=False,
        decoder=False).get_encoder_flops(batch_size, train_steps)

    dec = TransformerHparams(
        h=h_d, l=l_d, s=s, v=v, e=e, i=None, heads=None,
        head_size=None, output_frac=output_frac, sparse_embed_lookup=False,
        decoder=False).get_train_flops(batch_size, train_steps)

    cls = TransformerHparams(
        h=h_d, l=l_d, s=s, v=v, e=e, i=None, heads=None,
        head_size=None, output_frac=output_frac, sparse_embed_lookup=False,
        decoder=False).get_train_flops(batch_size, train_steps, True)

    print("flops of enc {}, dec {}, cls {}".format(enc, dec, cls))
    return enc + dec, enc + dec + cls
...
MODEL_FLOPS = collections.OrderedDict([
 ...
 ("cmlm, atreview", get_cmlm_train_flops(h_d=512, l_d=6, h_g=512, l_g=6, batch_size=1, train_steps=1, tied_embeddings=True,
                                      e=None, v=34984, s=128, output_frac=0.5))
])
